...
diceRouter.get("/:side", async (req: Request, res: Response) => {
  try {
    ...
    if(isNaN(side)) throw "Le dé lancé doit être un nombre";
      
    if(side === 0) throw "Le dé lancé doit être supérieur à zéro";
      
    const dice: T_Dice = await diceService.create(side);
